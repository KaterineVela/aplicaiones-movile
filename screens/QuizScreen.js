...
// Posiciones en español (Sociales)
...
export default function QuizScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (vozActiva && preguntaActual) {
      ...
      if (esIngles) {
        hablar(
          `Question ${numeroActual} of ${totalPreguntas}. ${contexto}` +
          `${preguntaActual.pregunta}. ` +
          `The options are: ${opcionesVoz}`
        );
      } else {
        hablar(
          `Pregunta ${numeroActual} de ${totalPreguntas}. ${contexto}` +
          `${preguntaActual.pregunta}. ` +
          `Las opciones son: ${opcionesVoz}`
        );
      }
    }
  }, [index, vozActiva]);
  ...
  const seleccionarRespuesta = (opcion) => {
    ...
    if (index < banco.length - 1) {
      ...
      navigation.navigate("Resultados", { respuestas: nuevas, materia });
    }
  };
  ...
  const leerPreguntaDeNuevo = () => {
    ...
    if (esIngles) {
      hablar(
        `Question ${numeroActual} of ${totalPreguntas}. ${contexto}` +
        `${preguntaActual.pregunta}. The options are: ${opcionesVoz}`
      );
    } else {
      hablar(
        `Pregunta ${numeroActual} de ${totalPreguntas}. ${contexto}` +
        `${preguntaActual.pregunta}. Las opciones son: ${opcionesVoz}`
      );
    }
  };
  ...
  const leerContexto = () => {
    if (preguntaActual.contexto) {
      hablar(esIngles
        ? `Context: ${preguntaActual.contexto}`
        : `Contexto: ${preguntaActual.contexto}`
      );
    } else {
      hablar(esIngles
        ? "This question has no additional context."
        : "Esta pregunta no tiene contexto adicional."
      );
    }
  };
    ...
    <View style={styles.wrapper}>
      ...
      <LinearGradient colors={colorMateria} style={styles.header}>
        <TouchableOpacity
          ...
          onPress={toggleVoz}
          accessibilityLabel={vozActiva
            ? (esIngles ? "Turn off voice" : "Desactivar voz")
            : (esIngles ? "Turn on voice" : "Activar voz")
          }
        >
        ...
        <View style={styles.card}>
          ...
          <View style={styles.botonesAudio}>
            <TouchableOpacity
              ...
              onPress={leerPreguntaDeNuevo}
              accessibilityLabel={esIngles ? "Listen to the question again" : "Escuchar pregunta de nuevo"}
            >
              <Text style={styles.audioBtnText}>
                🔊 {esIngles ? "Listen" : "Escuchar"}
              </Text>
            </TouchableOpacity>

            {preguntaActual.contexto && (
              <TouchableOpacity
                style={[styles.audioBtn, styles.audioBtnContexto]}
                onPress={leerContexto}
                accessibilityLabel={esIngles ? "Listen to the context" : "Escuchar contexto"}
              >
                <Text style={styles.audioBtnText}>
